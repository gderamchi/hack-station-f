    ...
    if (callNow) {
      ...
      if (retellClient) {
        try {
          ...
          // Use existing agent configured in Retell dashboard
          const agentId = process.env.RETELL_AGENT_ID!;
          const retellPhone = process.env.RETELL_PHONE_NUMBER!;

          console.log('✅ Using Retell agent:', agentId);
          console.log('✅ Calling from:', retellPhone);
          console.log('✅ Calling to:', testPhone);

          // Make the call with Retell AI using YOUR configured agent
          const retellCall = await retellClient.makeCall({
            agentId: agentId,
            toNumber: testPhone,
            fromNumber: retellPhone,
          });
